<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Document</title>
	<link rel="stylesheet" href="./ArticleHome.css">

</head>
<header class="nav-header">
	<a href="#" class="logo">𓄿𓄿𓂋𓅲</a>
	<nav class="navbar">
		<ul>
			<li><a href="../HomePage.php" class="navbar_text">Home</a></li>
			<li><a href="../circleNav.php" class="navbar_text">circleNav</a></li>
			<li><a href="../BookingPage.php" class="navbar_text">book trip</a></li>

		</ul>
	</nav>
	<a href="#"><button class="aboutus_butt">about us</button></a>
</header>

<ul class="card-list">

	<li class="card">
		<a class="card-image-nef" href="./KINGS,QUEENS,ETC../King Khufu.php" target="_blank">
			<img class="shape" src="../PICS/King Khufu/Portrait King khufu.svg" alt="let's go" />
		</a>
		<a class="card-description" href="./KINGS,QUEENS,ETC../King Khufu.php" target="_blank">
			<h2>King Khufu</h2>
		</a>
	</li>

	<li class="card">
		<a class="card-image-nef" href="./KINGS,QUEENS,ETC../Queen  Nefertiti.php" target="_blank">
			<img class="shape" src="../PICS/Queen Nefertiti/Queen Nefertiti 2.webp" alt="let's go" />
		</a>
		<a class="card-description" href="./KINGS,QUEENS,ETC../Queen  Nefertiti.php" target="_blank">
			<h2>Queen Nefertiti</h2>
		</a>
	</li>

	<li class="card">
		<a class="card-image-nef" href="./KINGS,QUEENS,ETC../Ramses.php" target="_blank">
			<img class="shape" src="../PICS/Ramses II/Ramses1.jpeg" alt="let's go" />
		</a>
		<a class="card-description" href="./KINGS,QUEENS,ETC../Ramses.php" target="_blank">
			<h2>Ramsees</h2>
		</a>
	</li>
	<ul class="card-list">

		<li class="card">
			<a class="card-image-nef" href="../King Khufu.HTML" target="_blank">
				<img class="shape" src="../PICS/King Khufu/Portrait King khufu.svg" alt="let's go" />
			</a>
			<a class="card-description" href="../Queen  Nefertiti.HTML" target="_blank">
				<h2>King Khufu</h2>
			</a>
		</li>

		<li class="card">
			<a class="card-image-nef" href="../Queen  Nefertiti.HTML" target="_blank">
				<img class="shape" src="../PICS/Queen Nefertiti/Queen Nefertiti 2.webp" alt="let's go" />
			</a>
			<a class="card-description" href="../Queen  Nefertiti.HTML" target="_blank">
				<h2>Queen Nefertiti</h2>
			</a>
		</li>

		<li class="card">
			<a class="card-image-nef" href="../Ramses.HTML" target="_blank">
				<img class="shape" src="../PICS/Ramses II/Ramses1.jpeg" alt="let's go" />
			</a>
			<a class="card-description" href="../Queen  Nefertiti.HTML" target="_blank">
				<h2>Ramsees</h2>
			</a>
		</li>


	</ul>
	</body>

</html>